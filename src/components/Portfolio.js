import { Fragment, useState } from "react";
import DetailsPopup from "./popup/DetailsPopup";

const Portfolio = () => {
  const [popup, setPopup] = useState(false);
  return (
    <Fragment>
      <DetailsPopup open={popup} close={() => setPopup(false)} />
      <div className="devman_tm_section" id="portfolio">
        <div className="devman_tm_portfolio">
          <div className="container">
            <div className="devman_tm_main_title" data-text-align="center">
              <span>Portfolio</span>
              <h3>Mis trabajos</h3>
              <p>Algunos de los proyectos en los que estoy actualmente involucrado
              </p>
            </div>
            <div className="portfolio_list">
              <ul>
                <li className="wow fadeInUp" data-wow-duration="1s">
                  <div className="list_inner">
                    <div
                      className="background_image"
                      data-img-url="img/portfolio/Mpaz.png"
                    />
                    <div className="content">
                      <div className="details">
                        <span className="category">
                          <a href="#">Imagen de referencia </a>
                        </span>
                        <h3 className="title">
                          <a href="">
                            Desarrollo web  <br /> Para Escultora 
                           <span> MPaz</span>
                          </a>
                        </h3>
                        <span className="view_project">
                          <a href="https://www.instagram.com/m.paz.kaplan/">
                            Ver sus obras <i className="icon-right-big" />
                          </a>
                        </span>
                      </div>
                    </div>
                    <div className="overlay" />
                    <a
                      className="devman_tm_full_link popup-youtube"
                      href="https://www.youtube.com/watch?v=7e90gBu4pas"
                    />
                  </div>
                </li>
                <li className="wow fadeInUp" data-wow-duration="1s">
                  <div className="list_inner">
                    <div
                      className="background_image"
                      data-img-url="img/portfolio/Full.png"
                    />
                    <div className="content">
                      <div className="details">
                        <span className="category">
                          <a href="#">FullCollege </a>
                        </span>
                        <h3 className="title">
                          <a href="#">
                            Aplicación Mobile 
                            <br /> 
                          </a>
                        </h3>
                        <span className="view_project">
                          <a href="#">
                             Descargar <i className="icon-right-big" />
                          </a>
                        </span>
                      </div>
                    </div>
                    <div className="overlay" />
                  
                  </div>
                </li>
                <li className="wow fadeInUp" data-wow-duration="1s">
                  <div className="list_inner">
                    <div
                      className="background_image"
                      data-img-url="img/portfolio/2.png"
                    />
                    <div className="content">
                      <div className="details">
                        <span className="category">
                          <a href="#">youtube</a>
                        </span>
                        <h3 className="title">
                          <a href="#">
                           Integrante activo <br></br>de la comunidad de desarrollo. 🚀
                          </a>
                        </h3>
                        <span className="view_project">
                          <a href="#">
                            Ver  <i className="icon-right-big" />
                          </a>
                        </span>
                      </div>
                    </div>
                    <div className="overlay" />
                    <a
                      className="devman_tm_full_link popup-vimeo"
                      href="https://www.youtube.com/watch?v=G0yhvyoFKwM"
                    />
                  </div>
                </li>
               
                
                <div
                  className="shape_1 moving_effect"
                  data-direction="y"
                  data-reverse="yes"
                />
                <div
                  className="shape_2 moving_effect"
                  data-direction="y"
                  data-reverse="yes"
                />
              </ul>
            </div>
          </div>
        </div>
      </div>
    </Fragment>
  );
};
export default Portfolio;
